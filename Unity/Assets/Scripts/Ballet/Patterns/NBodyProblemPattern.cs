    ...
    public class NBodyProblemPattern : DancePattern
    {
        ...
        override protected List<Vector3> getPatternPositions<T>(DancerGroup<T> group)
        {
            ...
            for (int i = 0; i < group.items.Count; i++)
            {
                ...
                group.items[i].velocity += acceleration * Time.deltaTime * group.patternSpeed;
                positions.Add(prevPositions[i] + group.items[i].velocity * Time.deltaTime * group.patternSpeed);
